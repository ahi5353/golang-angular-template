...
- **ログイン:**
    ...
    - **既にログイン済みのユーザーがログイン画面にアクセスした場合、自動的に「ダッシュボード画面」へリダイレクトする。**
- **ログアウト:**
...
- **新規追加:**
    - アカウント一覧画面の「追加」ボタンからアカウント作成ダイアログを開き、新しいユーザーを作成する。

## 3. 各画面の定義

### 3.1. 共通レイアウト

- **目的:** 認証後の各画面で共通のヘッダーを表示する。
- **適用範囲:** ダッシュボード、アカウント関連画面、サンプル画面。
- **ヘッダー要素:**
    - 左側: アプリケーションタイトル
    - 中央: ナビゲーションメニュー
        - ダッシュボード
...
- **表示条件:**
    - 登録ユーザー数が0件の時に、ルートパス(`/`)アクセスで自動的に表示される。
    - 既存ユーザーが存在する場合は表示できない（API側でエラーとする）。
    - **既にログイン済みのユーザーがアクセスした場合、「ダッシュボード画面」へリダイレクトする。**
- **画面要素:**
    - ユーザー名入力フィールド
    - パスワード入力フィールド
    - パスワード確認用フィールド
...
- **機能:**
    - 登録されている全ユーザーを表示する。
    - 「追加」ボタン押下で「アカウント作成ダイアログ」を開く。
    - 「削除」ボタン押下で「確認ダイアログ」を表示し、承認後にユーザーを削除する。

### 3.6. アカウント作成ダイアログ

- **目的:** ログイン済みユーザーが新しいアカウントを作成する。
- **表示条件:** アカウント一覧画面の「追加」ボタン押下時にモーダル表示。
- **画面要素:**
    - ユーザー名入力フィールド
    - パスワード入力フィールド
    - 作成ボタン
...
- **機能:**
    ...
    - 作成完了またはキャンセル時、ダイアログを閉じ、一覧画面の情報を更新する。

### 3.7. サンプル画面 (`/sample`)

- **目的:** 開発の参考用として、Angular Material (M3) のフォントサイズやコンポーネントの一覧を表示する。
- **表示条件:** ログイン後にヘッダーメニューの「サンプル」から遷移。
- **画面要素:**
    - タイポグラフィ一覧 (Display, Headline, Title, Body, Labelなど)
    - コンポーネント一覧 (Buttons, Cards, Inputs, Checks, Radioなど)
